...
?>
        ...
        <div id="<?php echo $this->get_field_id('title'); ?>_div" style="display: block;">
            ...
            <p><label for="<?php echo $this->get_field_id('uri'); ?>"><?php _e('Feedburner feed URL:','creativeMag'); ?> <input class="widefat" id="<?php echo $this->get_field_id('uri'); ?>" name="<?php echo $this->get_field_name('uri'); ?>" type="text" value="<?php echo $uri; ?>" /></label></p>          
            <p><label for="<?php echo $this->get_field_id('above_email'); ?>"><?php _e('Above input text:','creativeMag'); ?> <input class="widefat" id="<?php echo $this->get_field_id('above_email'); ?>" name="<?php echo $this->get_field_name('above_email'); ?>" type="text" value="<?php echo $above_email; ?>" /></label></p> 
            <p><label for="<?php echo $this->get_field_id('email_text_input'); ?>"><?php _e('Input placeholder text:','creativeMag'); ?> <input class="widefat" id="<?php echo $this->get_field_id('email_text_input'); ?>" name="<?php echo $this->get_field_name('email_text_input'); ?>" type="text" value="<?php echo $email_text_input; ?>" /></label></p> 
        </div>
...
<?php
    ...
    function generate($args, $instance) {
        ...
        if ($uri && count($queryParams) > 0) {
            ...
            // Open Form
            $html .= '<form class="mail-form" id="' . trim($form_id) . '" action="http://feedburner.google.com/fb/a/mailverify" method="post" onsubmit="' . implode('', $onsubmit) . '" target="popupwindow">';
			$html .= '<fieldset>';
			$html .= '<div class="head"><h3>Newsletter</h3>';
            if (trim($above_email)) {
                $html .= '<label for "text1">' . trim($above_email)  . '</label>';
            }
                else
                { $html .= '<label for "text1">Enter your email address and receive the latest articles in your mailbox:</label>';}
            $html .= '</div><div class="row"><input id = "text1" id="' . trim($form_id) . '_email" class = "text" name="email" type="text"  ';
            if(!empty($email_text_input)) {
                $html .= 'value="' . htmlentities(trim($email_text_input)) . '" placeholder="' . htmlentities(trim($email_text_input)) . '" ';
            }
            $html .= '/>';
            // Hidden fields
            ...
            $html .= '<input id="' . trim($form_id) . '_submit" class="submit" type="submit"  />';
